...
create table app_users (
  ...
  phone         text,                        -- collected during onboarding wizard
  username      text,                        -- lowercased IG handle; used for username login (unique, see idx_app_users_username)
  created_at    timestamptz default now(),
...
create table influencer_profiles (
  ...
  coverage_cities jsonb default '[]'::jsonb,  -- cities the creator covers (max 3, from locations table)
  creator_categories jsonb default '[]'::jsonb, -- creator's chosen category slugs (taxonomy in creator_categories table)
  available     boolean default true,         -- creator-facing availability toggle
  cover_url     text,                          -- profile cover image (Supabase Storage)
...
create table business_profiles (
  ...
  category        text,                        -- food_drinks | service | product
  target_creator_size jsonb default '[]'::jsonb, -- ["nano","micro",...] from prefs step
  target_budget   text,                        -- "micro" | "mid" | "big" from prefs step
  approved        boolean default false,       -- set true by admin after review
  created_at    timestamptz default now(),
  updated_at    timestamptz default now()
);
...
create table pitches (
  ...
  brief         text,
  budget        numeric(10,2),
  title         text,                          -- proposal title (UI payload)
  deliverables  jsonb default '[]'::jsonb,     -- [{label,qty,emoji}] (UI payload)
  counter       numeric(10,2),                 -- last counter amount
  from_role     text default 'business',       -- 'creator' proposal | 'business' pitch
...
-- NOTE: `credit_ledger` is the BUSINESS ledger. The creator-side ledger lives in
-- `creator_credit_ledger` (see below) — the names must not collide.
...
-- ---------- creator credit economy (migrations 20260613000010 / 012) --------
...
create table deals (
  ...
  title         text,                        -- copied from pitch (UI payload)
  deliverables  jsonb default '[]'::jsonb,   -- [{label,qty,emoji}] (UI payload)
  counter       numeric(10,2),               -- last counter amount
  submission    jsonb,                       -- {emoji,from,to,caption,link,likes,views,comments}
  review        jsonb,                       -- {rating,text}
  log           jsonb default '[]'::jsonb,   -- LogEvent[] rendered timeline
  created_at    timestamptz default now(),
  updated_at    timestamptz default now()
);

-- ---------- campaigns (business briefs surfaced to creators) ---------------
create table campaigns (
  id            uuid primary key default gen_random_uuid(),
  business_id   uuid not null references business_profiles(id) on delete cascade,
  title         text not null,
  category      text,                              -- CAT_MAP key or niche slug
  blurb         text,
...
create index idx_campaigns_biz        on campaigns(business_id, status);
